        ...
        {colors.map((c) => (
          <button
            ...
            className={`w-6 h-6 rounded-md border-2 transition-all ${
              selected === c.value ? "border-white scale-110" : "border-transparent hover:border-white/30"
            }`}
...
export default function TaglinesTab({
  ...
  const taglineFontFamily = (data.tagline_font_family as string) || "poppins";
  ...
  const taglineBgOpacity = (data.tagline_background_opacity as number) ?? 100;
  const taglineCharLimit = (data.tagline_char_limit as number) || 0;
  const taglineCharLimitCustom = (data.tagline_char_limit_custom as number) || 100;
  const taglineWrapEnabled = (data.tagline_wrap_enabled as boolean) !== false;
  const taglineWrapMaxChars = (data.tagline_wrap_max_chars as number) || 0;
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                type="number"
                value={(data.tagline_top_margin as number) || 80}
                onChange={e => onChange("tagline_top_margin", parseInt(e.target.value) || 80)}
                min="0"
                max="500"
                className="w-full px-2 py-1.5 bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.08)] rounded-lg text-[11px] text-white focus:outline-none"
              />
            ...
            <div>
              ...
              <input
                type="number"
                value={(data.tagline_bottom_margin as number) || 80}
                onChange={e => onChange("tagline_bottom_margin", parseInt(e.target.value) || 80)}
                min="0"
                max="500"
                className="w-full px-2 py-1.5 bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.08)] rounded-lg text-[11px] text-white focus:outline-none"
              />
            ...
            {taglineWrapEnabled && (
              <div className="mt-1 flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={e => onChange("tagline_wrap_max_chars", parseInt(e.target.value) || 0)}
                  placeholder="Auto"
