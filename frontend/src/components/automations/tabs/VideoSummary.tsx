...
const featureColors: Record<string, { bg: string; text: string }> = {
  ...
  Prompt: { bg: "rgba(34,211,238,0.2)", text: "#67e8f9" },
};
...
export default function VideoSummary({ data }: Props) {
  ...
  if (data.short_generation_mode === "prompt") features.push("Prompt");
  const promptSourceLabel = data.short_generation_mode === "prompt"
    ? String(data.prompt_source_type || "youtube").replace("_", " ")
    : null;
  ...
  return (
    ...
    >
      ...
      {promptSourceLabel && (
        <div className="flex items-center justify-between text-[10px] mt-1">
          <span className="text-[#a1a1aa]">Prompt Source:</span>
          <span className="text-white font-medium capitalize">{promptSourceLabel}</span>
        </div>
      )}
    </div>
  );
