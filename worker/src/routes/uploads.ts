import { Env, VideoUpload, PostformeSettings } from "../types";
import { jsonResponse } from "../utils";

async function deleteCompletedJob(env: Env, jobId: number): Promise<void> {
  const job = await env.DB.prepare("SELECT id, status FROM jobs WHERE id = ?").bind(jobId).first<{ id: number; status: string }>();
...
export async function handleUploadsRoutes(
  ...
  if (id && !action) {
    ...
    if (method === "DELETE") {
      const upload = await env.DB.prepare("SELECT job_id FROM video_uploads WHERE id = ?").bind(id).first<{ job_id: number }>();
      await env.DB.prepare("DELETE FROM video_uploads WHERE id = ?").bind(id).run();
      if (upload?.job_id) {
        await deleteCompletedJob(env, upload.job_id);
      }
